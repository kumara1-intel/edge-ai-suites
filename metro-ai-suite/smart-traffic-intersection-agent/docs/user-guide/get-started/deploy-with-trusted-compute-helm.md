...
run it as an OpenShell sandbox. Combined with the `kata-qemu` runtime class, this adds a second
isolation layer on top of Trusted Compute: a Landlock filesystem policy and enforced L7 egress.

See [Run the Traffic Agent as an OpenShell Sandbox](./deploy-with-openshell-sandbox.md).
...
**Step 4. Remove the OpenShell Sandbox** (if deployed with `openshell.enabled=true`):

See the [cleanup steps](./deploy-with-openshell-sandbox.md#clean-up) in the OpenShell guide.
